...
const getDensityComparison = async () => {
  ...
  const calculateFlow = (totalTime, count) => {
    if (!count || count === 0) return "0.00s";
  ...
  return {
    currentHour: {
      ...
      trafficFlow: calculateFlow(current.totalTravelTime, current.vehicleCount),
    ...
    previousHour: previous
      ? {
          ...
          trafficFlow: calculateFlow(
            previous.totalTravelTime,
            previous.vehicleCount
          ),
