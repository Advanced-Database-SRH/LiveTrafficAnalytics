...
function App() {
	...
	async function fetchTrafficData() {
		try {
			const [eventsRes, historyRes, hourlyRes] = await Promise.all([
				fetch(EVENTS_API),
				fetch(HISTORY_API),
				fetch(HOURLY_ANALYTICS_API),
			]);

			if (!eventsRes.ok || !historyRes.ok || !hourlyRes.ok)
				throw new Error("Backend request failed");

			setEvents(await eventsRes.json());
			setHistory(await historyRes.json());
			setHourlyAnalytics(await hourlyRes.json());
			setError("");
		} catch (err) {
	...
	const vehicleBreakdown = useMemo(() => {
		const counts = hourlyAnalytics[0]?.counts || {};
		return {
			car: counts.car || 0,
			bus: counts.bus || 0,
	...
	const totalVehicles = useMemo(
		() => Object.values(vehicleBreakdown).reduce((sum, v) => sum + v, 0),
		[vehicleBreakdown],
	);
	...
	return (
		<main className="min-h-screen bg-[#eef3f8] text-slate-950">
			<div className="mx-auto max-w-[1500px] px-5 py-8 md:px-8">
				...
				{/* Traffic Overview + Vehicle Breakdown */}
